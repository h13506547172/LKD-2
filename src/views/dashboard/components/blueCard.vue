<template>
  <div class="boxTopOne">
    <img
      class="cityPic"
      src="http://likede2-admin.itheima.net/img/task.66b715b7.png"
      alt=""
    />
    ...
    <div class="content">
      <div class="contentItem">
        <p>{{ total }}</p>
        <span class="small">工单总数（个）</span>
      </div>
      <div class="contentItem">
        <p>{{ completedTotal }}</p>
        <span class="small">完成工单（个）</span>
      </div>
      <div class="contentItem">
        <p>{{ progressTotal }}</p>
        <span class="small">进行工单（个）</span>
      </div>
      <div class="contentItem">
        <p>{{ cancelTotal }}</p>
        <span class="small">取消工单（个）</span>
      </div>
    </div>
  </div>
</template>
...
import { getReportInfo } from '@/api/home'
export default {
  data() {
    return {
      //工单统计↓
  ...
  methods: {
    ...
    async getReportInfo() {
      ...
      const { data } = await getReportInfo(start, end)
      data.forEach((item) => {
